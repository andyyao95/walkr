...
\abstract{
The \texttt{walkr} package uses random walks to sample points from the intersection of 
...
walks. \texttt{walkr} also provide tools to examine the convergence
...
Dikin Walk, which generates a nearly uniform sample and exhibits much stronger mixing. 
\code{walkr} uses \code{Rcpp}(\cite{rcpp}) and \code{RcppEigen}(\cite{rcppeigen}) 
for its core matrix calculations which take up most of the run-time.
...
\begin{alltt}
\hlstd{z} \hlkwb{<-} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{1}\hlstd{,} \hlnum{0.0001}\hlstd{)}

\hlkwd{plot}\hlstd{(}\hlopt{-}\hlkwd{log}\hlstd{(z)}\hlopt{~}\hlstd{z,} \hlkwc{type} \hlstd{=} \hlstr{'l'}\hlstd{,} \hlkwc{ylim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{5}\hlstd{))}
\end{alltt}
\end{kframe}
...
\hlkwd{plot}\hlstd{((}\hlnum{1}\hlopt{/}\hlstd{z}\hlopt{^}\hlnum{2}\hlstd{)}\hlopt{~}\hlstd{z,} \hlkwc{type} \hlstd{=} \hlstr{'l'}\hlstd{,} \hlkwc{ylim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{50}\hlstd{))}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/example1_5-1} 
...
itself as it surveys through the polytope $K$, biasing away from the corners of the region(\cite{kannan}).
...
\begin{verbatim}
##            [,1]      [,2]      [,3]      [,4]      [,5]       [,6]
## [1,] 0.35058592 0.5177956 0.3102097 0.1475507 0.2403375 0.10648788
## [2,] 0.62385906 0.2563244 0.5165506 0.5520851 0.6065870 0.86594311
## [3,] 0.02555502 0.2258801 0.1732397 0.3003641 0.1530755 0.02756901
##           [,7]       [,8]       [,9]      [,10]
## [1,] 0.1115985 0.03108041 0.34387887 0.23228883
## [2,] 0.7826410 0.04414081 0.64365691 0.73630416
## [3,] 0.1057605 0.92477878 0.01246422 0.03140701
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{A} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}\hlkwd{c}\hlstd{(}\hlnum{2}\hlstd{,}\hlnum{0}\hlstd{,}\hlnum{3}\hlstd{,}\hlnum{0}\hlstd{),} \hlkwc{ncol} \hlstd{=} \hlnum{4}\hlstd{)}
\hlstd{b} \hlkwb{<-} \hlnum{0.5}
\hlkwd{set.seed}\hlstd{(}\hlnum{314}\hlstd{)}
\hlstd{z1} \hlkwb{<-} \hlkwd{walkr}\hlstd{(}\hlkwc{A} \hlstd{= A,} \hlkwc{b} \hlstd{= b,} \hlkwc{points} \hlstd{=} \hlnum{1000}\hlstd{,} \hlkwc{method} \hlstd{=} \hlstr{"dikin"}\hlstd{)}
\hlstd{z2} \hlkwb{<-} \hlkwd{walkr}\hlstd{(}\hlkwc{A} \hlstd{= A,} \hlkwc{b} \hlstd{= b,} \hlkwc{points} \hlstd{=} \hlnum{1000}\hlstd{,} \hlkwc{method} \hlstd{=} \hlstr{"hit-and-run"}\hlstd{)}
\hlkwd{library}\hlstd{(scatterplot3d)}
\hlkwd{scatterplot3d}\hlstd{(}\hlkwc{x} \hlstd{= z1[}\hlnum{2}\hlstd{,],} \hlkwc{y} \hlstd{= z1[}\hlnum{3}\hlstd{, ],} \hlkwc{z} \hlstd{= z1[}\hlnum{4}\hlstd{, ],} \hlkwc{pch} \hlstd{=} \hlnum{19}\hlstd{,} \hlkwc{scale.y} \hlstd{=} \hlnum{2}\hlstd{,} \hlkwc{main} \hlstd{=} \hlstr{"Dikin"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/example8-1} 
\begin{kframe}\begin{alltt}
\hlkwd{scatterplot3d}\hlstd{(}\hlkwc{x} \hlstd{= z2[}\hlnum{2}\hlstd{,],} \hlkwc{y} \hlstd{= z2[}\hlnum{3}\hlstd{, ],} \hlkwc{z} \hlstd{= z2[}\hlnum{4}\hlstd{, ],} \hlkwc{pch} \hlstd{=} \hlnum{19}\hlstd{,} \hlkwc{scale.y} \hlstd{=} \hlnum{2}\hlstd{,} \hlkwc{main} \hlstd{=} \hlstr{"Hit-and-Run"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/example8-2} 

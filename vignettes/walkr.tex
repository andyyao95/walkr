...
The log-barrier function of $Ax \leq b$ measures how "extreme" or "close-to-the-boundary" a point $x$ is 
within the polytope $K$. This is because the value of $\phi$ tends to $\infty$ as $x$ tends to the 
...
\begin{alltt}
\hlstd{x} \hlkwb{<-} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{1}\hlstd{,} \hlnum{0.0001}\hlstd{)}

\hlkwd{plot}\hlstd{(}\hlopt{-}\hlkwd{log}\hlstd{(x)}\hlopt{~}\hlstd{x,} \hlkwc{type} \hlstd{=} \hlstr{'l'}\hlstd{,} \hlkwc{ylim} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{10}\hlstd{))}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/example1-1} 

\end{knitrout}

Thus, the higher the value of the log-barrier function ($\phi(x)$), the closer the point $x$ is to the boundary
of the convex polytope $K$. \\
...
The Hessian matrix($H_x = \nabla ^2 \phi(x)$) is a matrix containing the second derivatives 
of the function $\phi(x)$ with respect to $x$. In the land of optimization, the Hessian contains 
information on how the landscape is shaped. Thus, this specific Hessian measures how quickly 
the log-barrier function $\phi(x)$ is changing around the current point $x$. 
...
"Hessian norm" as the notion of length in this space. The "Hessian norm" we could think as way of 
quantifying how quickly approaching the boundary a point in the polytope is. In other words,
the Dikin Ellipsoid is the collection of all points whose difference with the current point($y-x_0$) 
is within the unit threshhold of $r=1$. \\

The virtue of the Dikin Ellipsoid is that it is biased away from the corners of the polytope $K$(\cite{kannan}).
Moreover, because the Dikin Ellipsoid is the collection of all the points whose "change in closeness to 
the boundary" is within a threshold, the Dikin Ellipsoid changes shapes as the point $x$ surveys through the space.
Namely, when the center $x$ of the Dikin Ellipsoid is far from the boundary
of the polytope, the ellipsoid is bigger. As the point 
$x$ approaches the boundary of the polytope, the ellipsoid becomes smaller and smaller. Therefore, 
the ellipsoid is able to reshape itself as it surveys through the polytope $K$, biasing away from the corners
of the region.
...
\begin{verbatim}
##           [,1]      [,2]      [,3]      [,4]      [,5]      [,6]      [,7]
## [1,] 0.4256675 0.5245698 0.1959297 0.1767314 0.2740561 0.2529715 0.2575030
## [2,] 0.4465196 0.1714794 0.6279739 0.6019743 0.1532165 0.1484322 0.3881598
## [3,] 0.1278129 0.3039507 0.1760964 0.2212943 0.5727274 0.5985964 0.3543371
##           [,8]       [,9]      [,10]
## [1,] 0.2440586 0.42755488 0.35231926
## [2,] 0.3419459 0.57112834 0.61130192
## [3,] 0.4139955 0.00131678 0.03637882
\end{verbatim}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in hello.world[, what] <- y: number of items to replace is not a multiple of replacement length}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xy.coords(x, y, xlabel, ylabel, log): object 'answer\_dikin' not found}}\end{kframe}
\end{knitrout}

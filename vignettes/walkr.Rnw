...
\begin{enumerate}
  ...
  \item{If $x_0 \in D_{y}$, then accept $y$ with probability $\min(1, \sqrt{\frac{det(H_{y})}{det(H_{x_0})}})$.
        ...
        } x_0 }{\text{volume of Dikin ellipsoid centered at } y}$. % need to add: why the transition probability according to volume ratio
        (We need not worry about the accepted point $y$

...
unthorough sampling, so the random walks need to be run longer to cover more regions. 
\pkg{walkr} examines the quality of MCMC samples.
...
This space is now described in terms of $\alpha_i$'s, coefficients of the basis vectors. 

...
\begin{enumerate}
  ...
  \item{If $x_0 \in D_{y}$, then accept $y$ with probability $\min(1, \sqrt{\frac{det(H_{y})}{det(H_{x_0})}})$.
        ...
        we accept $y$ depending on the ellipsoids' volumes.  
...
to escape the corner and reach other parts of the sample space. The Stan Manual (\cite{stanmanual})
provides a detailed explaination of techniques to improve MCMC sampling. Stan is a probabilistic 
programming language that implements Bayesian inference models. 
...
Figure~\ref{fig:shinystan} shows a screenshot of a traceplot from the \pkg{shinystan} interface.

...
In this context, the Hessian $H_{x_0}$ at $x_0$ is used as a norm, which we call the ``Hessian norm''.
Therefore, the Dikin Ellipsoid we can think of the collection of all the points around 
...
the larger the value of the ``Hessian norm'', and thus, the fewer number of points allowed within
the unit threshold, which leads to a smaller Dikin ellipsoid. On the other hand, the further 
...
$-\log(x)$. The Hessian is the generalized second derivative, and the second derivative of $-\log(x)$
is $\frac{1}{x^2}$. Therefore, for vector $v$, Hessian norm at point $x$ would be 
$||v||^2_{x_0} = \frac{v^2}{x^2}$. The closer $x$ (recall that in our case, $x = a_i^Tx - b_i$) is to 
zero, the larger the norm. 

For example, Figure \ref{fig:dikinellip} shows a Dikin ellipsoid in the $3$D-simplex.

\begin{figure}[H]
\centering
\includegraphics[width = 3.5in, height = 2.5in]{img/dikinellip.png}
\caption[caption]{The Dikin ellipsoid centered at a point in the interior of the 
                  $3$D-simplex.}
\label{fig:dikinellip}  %% THIS ELLIPSOID WAS SAMPLED FROM ALPHAS AND THEN MAPPED BACK 
\end{figure} 
...
\begin{enumerate}
  ...
  \item{Begin with a point $x_0 \in K$. This starting point must be in the polytope and 
        not on the edge of polytope $K$. If $x_0$ is on the boundary, then $Ax_0 = b$,
        and consequently, the log-barrier and its Hessian would be infinity. Therefore, 
  ...
  \item{If $x_0 \in D_{y}$, then accept $y$ with probability $\min(1, \sqrt{\frac{det(H_{y})}{det(H_{x_0})}})$.
        ...
        Recall that the volume of the ellipsoid reflects how close to the boundary a point is. The closer a point
        is to the boundary, the smaller its Dikin ellipsoid volume. This transition probability is crucial
        becaues it avoids the algorithm concentrated in the ``center'' of the polytope. Because we already 
        ...
        (the ratio is greater than 1), we prefer this, so we move there with probability $1$. 
        If $x_0$ is closer to the boundary than $y$ (ratio smaller than 1), then
        we accept $y$ depending on how the ratio of the their Dikin ellipsoids' volumes.  % maybe add this as footnote
        ...
        will be fully contained in $K$ (see \cite{kannan} 2.1.4)}}
...
\caption{The Dikin Walk begins by constructing the Dikin Ellipsoid at the starting point $x_0$ (a).
         This point cannot be on the boundary of the polytope, otherwise the log-barrier and 
         its Hessian would both be infinity.
         An uniformly random point $y$ is generated in the Dikin Ellipsoid centered at $x_0$ (b). 
         If point $x_0$ is not in the Dikin Ellipsoid centered at $y$, then reject $y$ (c).
         If point $x_0$ is contained in the Dikin Ellipsoid centered at $y$, then  %%% elaborate
         accept $y$ with probability $\min(1, \sqrt{\frac{det(H_{y})}{det(H_{x_0})}})$ (d).
         Once we've successfully accepted $y$, we set $y$ as our new point, $x_1$ (e). 
         Algorithm repeats (f).} 
...
the mixing of Dikin is independent of the geometry of the polytope $K$, where as hit-and-run
mixes slower in ``skinny'' regions of the polytope, tending to get stuck in corners (\cite{kannan}).
However, the quick mixing comes with a cost of non-uniform sampling, as \cite{kannan} claims
that the Dikin walk is a ``nearly uniform'' sampling algorithm. 

Because the inherent nature of the log-barrier function and Hessian which prevents the Dikin
...
in the histograms, hit-and-run samples uniformly across 

<<example0, eval = T, fig.width = 3, fig.height = 3>>=
...
in its Dikin walk implementation for faster matrix multiplication, inversion, and determinant calculation,
because the majority of the run-time cost for Dikin walk is matrix operations. This improvement in 
run-time speed is important especially in sampling from $A$ with many columns, corresponding to polytopes in 
higher dimensions, because in such cases, a larger number of points needed to be sampled in order
to obtain a well mixed sample. 

To improve the mixing of the sample, there are in general two techniques. First, is to ``thin'' 
...
general, becomes slower. We note that thinning is effectively equivalent to sampling more points
...
to leave the corner and reach other parts of the sample space. 

To quantitatively examine the mixing, we can use the Gelman-Rubin diagnostic on multiple chains 
from diverse starting points. The general idea is that we measure the variance within each chain
...
($\hat{R}$) indicates that the chains should be ran for longer iterations.

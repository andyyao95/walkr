...
\textbf{Important:} Before we move on to sampling algorithms, we clear up on some nomenclature. 
First, as established above, the sampling space is a convex polytope. We shall refer to this convex 
polytope as $K$. Moreover, because 

\subsection{Picking a Starting Point}

MCMC random walks need a starting point, $x_0$, in the convex polytope. \texttt{walkr}
generates such starting points using linear programming.
Specifically, the \texttt{lsei} function of \texttt{limSolve} finds $x$ which: % cite the package

$$\text{minimizes} \quad |Cx-d|^2$$
$$\text{subject to} \quad Ax \leq b$$

We randomly generate $C$ and $d$, thereby obtaining $x$ which satisfy $Ax \leq b$. 
-walkr generates 30 points and then takes the average

note that we're sampling alphas 

note that we're taking average of those points. those points ought to be still in the convex polytope

talk about the complexity of picking starting points 
...
introduce chains 
we need different start pointings 

% NOTE: OUGHT TO UNDERSTAND AND EXPLAIN THE STARTING POINT BETTER 
% is there a literature about starting points? for MCMC 
...
%' %\code{hit-and-run} package on CRAN (\cite{hitandrun}).
%' 
%' walkr uses the har function from the hitandrun packge on CRAN (citation) to implement hit-and-run. 
%' 
%' %probably also want to discuss important facts about the algorithm here.
%' 
%' %examine properties of hit-and-run, include citation for 
%' %also talk about problems with hit-and-run
%' 
%' \section{Random Walk: Dikin Walk}
%' 
%' %Dikin walk is an alternatively way of sampling. It samples 
%' 
...
%' boundary of the polytope $K$. Mathematically, the polytope could be described by: %quote better
%' 
%' $$Ax \leq b$$
...
%' 
%' 
%' %% INCLUDE THIS IN THE DIKIN ELLIPSOID HELP PAGE
%' 
...
%' % \end{enumerate}
%' 
%' % \subsection{Important Theorem}   ==> JUST BE ONE SENTENCE IN ALGORITHM DIKIN
%' % 

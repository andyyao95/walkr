...
$-V\alpha \leq v_{particular}$. We sample $\alpha$'s.
...
which will usually fall on the boundaries of polytope $K$. We repeat this process 30 times 
...
\begin{enumerate}
  ...
  \item{If $x_0 \in D_{y}$, then accept $y$ with probability $\min(1, \sqrt{\frac{det(H_{y})}{det(H_{x_0})}})$.
        ...
        This transition probability is crucial because it avoids the algorithm from concentrating 
...
scatterplot3d(x = sampled_points[1,], y = sampled_points[2,], z = sampled_points[3,], 
              scale.y = 0.5, xlab = "x1", ylab = "x2", zlab = "x3", pch = 19)
@

...
independent without changing the solution space. We assume that the rows of $A$ are 
...
convex polytope. Sampling from such an object is a difficult problem, and the most common approach is to 
...
it is biased towards points that are away from the edges of $K$ (\cite{kannan}). 
...
\pkg{walkr} samples from this polytope using two Monte-Carlo
...
The major problem with our current implementation is that the run-time becomes unwieldy 

...
convex polytope. This convex polytope quickly becomes extremely complex, 
as its number of vertices grow combinatorically with $N$. Sampling from such a convex polytope 
is a difficult problem, and the common approach is to run Monte Carlo Markov Chains (MCMC) 
in the interior of the polytope. \\

\pkg{walkr} contains two MCMC random walks. Our first random walk is hit-and-run. 
...
\code{walkr} uses \code{Rcpp}(\cite{rcpp}) and \code{RcppEigen}(\cite{rcppeigen}) % skip space between Rcpp and \cite
for its core matrix calculations which take up most of the run-time.

% reference to the 2 algorithms itself belong in the introduction
% walkr package can be considered an extension to the hitandrun package 

\texttt{walkr} also provides statistical diagnostics of the mixing and convergence
properties of a MCMC random walk. 

% ^^^ make this a paragraph. How do you know mixing. how do you you're not getting into corners
% we generally use multiple chains 
% you can tell when this HAVENT mixed
% easier to tell its screwed up 
% harder to tell that its not screwed up at all

%  INSTEAD: \section{Simple Examples}

% in order to fix ideas, lets go through 3D and 4D examples
...
%' 
%' 
%' % THIS SECTION, JUST REMOVE. JUST EXPLAIN MORE CLEARLY IN INTRODUCTION
%' 
%' 
...
%' $$ 
%' 
%' % instead of "following graph" ==> in figure (\ref{figure label}), something like that 
%' 
...
%' \end{figure} % figure needs to have stand-alone caption, so make it more
%' 
...
%' \caption[caption]{4D Simplex Can Live in 3D Space} % NEED SENTENCES EXPLAINING THIS 
%' \end{figure} %4D simplex is something that theoretically exists in 4D space but can be viewed as a 3D object
%' 
...
%' 
%' % NEW SECTION
%' % GOING FROM X SPACE TO ALPHA SPACE 
%' 
...
%' Linear Algebra textbook). \\ % for review of concepts: linear algebra 
%' 
...
%' 
%' 
%' <<example1, eval = TRUE, fig.width=3, fig.height=3>>=
%' 
%' z <- seq(0, 1, 0.0001)
%' 
%' plot(-log(z)~z, type = 'l', ylim = c(0,5))
%' 
%' @
%' 
...
%' 
%' <<example1_5, eval = TRUE, fig.width = 3, fig.height = 3>>=
%' 
%' z <- seq(0, 1, 0.0001)
%' 
%' plot((1/z^2)~z, type = 'l', ylim = c(0,50))
%' 
%' @
%' 

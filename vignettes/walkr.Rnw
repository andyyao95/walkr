...
While good mixing does not guarantee a thorough sampling of the space, poor mixing means 
...
in order to make the connections to existing litearture more clear.
}. 
...
inequalities, $x_i \geq 0$. In this section, we present combine all three parts 
...
Once we have the null space, we add on a particular solution. Think of the null space as 
...
\begin{enumerate}
  ...
  \item{If $x_0 \in D_{y}$, then accept $y$ with probability $\min(1, \sqrt{\frac{det(H_{y})}{det(H_{x_0})}})$.
        ...
        This transition probability is crucial because it prevents algorithm from concentrating 
...
See Figure~\ref{fig:figure}. Dikin mixes much faster than hit-and-run does, 
...
in regions that are away from the boundary. 

%% The previous paragaph should end with, See Figure~/ref{blah} for an illustration. 

%% The below plots should be: 
%% side-by-side
%% With a better x-axis labels
%% With a single title covering both plots, probably something like: "Two Sampling Methods"
%% With titles covering each sub-plot, probably mentioning the specific algorithm
%% "2D-simplex" is a poor and confusing phrase to include in a title.
%% And the below text, with more detail, should go into the figure caption for the new figure,
%% explaining exactly where the points from and what they mean. 

We sample from the $2$D-simplex, which is a 
line segment ($x_1+x_2=1$, $x_i \geq 0$). As we can see in the histograms, 
...
<<example0, eval = T, fig.width = 3, fig.height = 3, cache = TRUE, echo = FALSE, message=FALSE, fig.show='hold'>>=
library(walkr)
A <- matrix(1, ncol = 2)
b <- 1 
set.seed(314)
...
qplot(sample_har[1,], xlab = "x1", main = "Hit-and-run: 2D-simplex")
qplot(sample_dikin[1,], xlab = "x1", main = "Dikin walk: 2D-simplex")
...
%% This paragraph could be improved, citing the Stan manual and giving a sentence or two of 
%% background. Specific parameter values for walkr should not be mentioned.
...
%include screenshot? Or at least a reference for shinystan.

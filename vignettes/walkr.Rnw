...
but indicates stronger mixing. \texttt{walkr} furthermore provides statistical diagnostics of 
...
We go through a few examples as well as some linear algebra 
...
Now that we've understood that the intersection of $Ax=b$ with the unit-simplex is a convex polytope, and 
that we could describe the polytope with $Ax \leq b$, we are ready to dive into the core of \texttt{walkr}
-- MCMC random walks.
...
currently generates starting points using a linear programming method implemented by \texttt{limSolve}.

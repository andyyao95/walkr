...
Find $x$'s that satisfy $A'x = b'$ by computing the null space of $A'$ and then adding on a particular solution. 
See \cite{leon} for a review of the specifics of finding null spaces and particular solutions.



% recall that the null space of A' is defined as all the x's that satisfy 
% Ax = 0 , and then particualr solution 
% we need the null space because of blah blah bllah 
% 

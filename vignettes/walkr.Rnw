...
%% This ought to be generated on the fly. Isn't the sampling in such a simple case
%% instantaneous? And then the figure (with two panels --- learn about par(mfrow)).
...
\label{fig:subfigure2}}
\caption{Consider the 2D-simplex, defined as $x_1+x_2=1$ and $x_1, x_2 \geq 0$.
         The 2D-simplex is a line-segment. The left panel shows . . .} 
         
%% This caption should be a full paragraph. Tells us what you did. Like how many points are 
%% sampled. Explain that you are just showing a cross-section of x_1. 
...
Dikin walk samples ``nearly uniformly'', avoiding 

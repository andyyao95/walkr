...
%% The previous paragaph should end with, See Figure~/ref{blah} for an illustration. And that
%% the below text, with more detail, should go into the figure caption for the new figure.
...
general, becomes slower. Thinning is equivalent to sampling more points
...
to escape the corner and reach other parts of the sample space. \pkg{walkr}'s default
parameters are 1 for thinning and 50\% for burn-in. 

...
# load libraries: cem is for CEM; the other two are for other matching methods
require(cem)
require(MatchIt)
require(Matching)

# a working subset of LeLonde that we'll be dealing with
data(DW)
...
# Tr.pred = probability, through the logit transformation
